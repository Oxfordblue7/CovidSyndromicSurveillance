...
def get_flagged_cuis_from_annotated_file (filepath):
    ...
    for index,row in f1.iterrows():
        if type(row['Symptom CUIs']) is float:
            continue
        id_ = row['ID']
...
# f1_flagged_cuis = get_flagged_cuis_from_annotated_file('./sample_annot1.xlsx')
# f2_flagged_cuis = get_flagged_cuis_from_annotated_file('./sample_annot_2.xlsx')
# f1_flagged_cuis = get_flagged_cuis_from_annotated_file('./Assignment1GoldStandardSet.xlsx')
# f2_flagged_cuis = get_flagged_cuis_from_annotated_file('../s4.xlsx')


def run(goldStd, annofile):

    f1_flagged_cuis = get_flagged_cuis_from_annotated_file(goldStd)
    f2_flagged_cuis = get_flagged_cuis_from_annotated_file(annofile)
